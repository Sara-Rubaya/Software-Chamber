...
const badges = [
  ...
  { text: "Website Development", position: "top-90 left-90 -translate-y-1" },
...
const serviceCards = [
  {
    ...
    image: "https://i.ibb.co/SLhPjZx/Illustration-5.png",
  },
  {
    id: 2,
    title: "Software Service Solutions",
    description: "Plan, track, and manage projects with precision tools.",
    image: "https://i.ibb.co/Z6PvdC50/Illustration-6.png",
  },
  {
    id: 3,
    title: "Software Service Solutions",
    description: "Plan, track, and manage projects with precision tools.",
    image: "https://i.ibb.co/bMNgdZ45/Illustration-7.png",
  },
];
...
<div className="relative w-full mt-20">
  {badges.map((badge, index) => (
    <div
      key={index}
      className={`absolute px-3 py-1 md:px-4 md:py-2 text-sm md:text-lg rounded-full bg-gray-800 border border-gray-700 text-white shadow-lg ${badge.position}
...
{/* Service Cards */}
<div
  className="relative mb-50 w-full sm:w-4/5 md:w-2/3 h-auto md:h-[300px] flex flex-col md:flex-row items-center justify-center card-container"
  onMouseEnter={() => {
    setHovered(true);
    setIsPaused(true);
  }}
...
>
  {serviceCards.map((card) => {
    ...
    return (
      <div
        key={card.id}
        className={`absolute w-full max-w-sm sm:max-w-md md:max-w-xl h-full transition-all duration-700 ease-in-out ${
      ...
      >
        <div className="card card-side flex-col sm:flex-row bg-base-100 shadow-sm rounded-[20px] h-auto md:h-full p-4 flex items-center">
          <figure className="flex-shrink-0">
            <img
              src={card.image}
              alt={card.title}
              className="w-full sm:w-50 h-40 object-cover rounded-t-[20px] sm:rounded-l-[30px] sm:rounded-t-none"
            />
          </figure>
          <div className="card-body p-4 flex flex-col justify-between w-full sm:max-w-[calc(100%-10rem)]">
            <h3 className="card-title text-md sm:text-lg md:text-xl">{card.title}</h3>
            <p className="mb-4 text-sm sm:text-base">{card.description}</p>
            <div className="card-actions justify-start">
              <button className="btn bg-teal-600 hover:bg-teal-950 px-4 py-1 text-sm sm:text-base">
                Discover
              </button>
            </div>
          </div>
